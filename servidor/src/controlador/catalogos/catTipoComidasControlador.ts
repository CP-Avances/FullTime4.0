...
class TipoComidasControlador {
  public async ListarTipoComidas(req: Request, res: Response) {
    const TIPO_COMIDAS = await pool.query(
      "SELECT ctc.id, ctc.nombre, ctc.tipo_comida, ctc.hora_inicio, " +
  ...
  public async VerUnMenu(req: Request, res: Response): Promise<any> {
    ...
    const TIPO_COMIDAS = await pool.query(
      "SELECT ctc.id, ctc.nombre, ctc.tipo_comida, ctc.hora_inicio, " +
        "ctc.hora_fin, tc.nombre AS tipo FROM cg_tipo_comidas AS ctc, tipo_comida AS tc " +
        "WHERE ctc.tipo_comida = tc.id AND ctc.id = $1",
      [id]
    );
    if (TIPO_COMIDAS.rowCount > 0) {
      return res.jsonp(TIPO_COMIDAS.rows);
  ...
  public async ListarUnTipoComida(req: Request, res: Response): Promise<any> {
    ...
    const TIPO_COMIDAS = await pool.query(
      "SELECT ctc.id, ctc.nombre, ctc.tipo_comida, ctc.hora_inicio, " +
        "ctc.hora_fin, tc.nombre AS tipo FROM cg_tipo_comidas AS ctc, tipo_comida AS tc " +
        " WHERE ctc.tipo_comida = tc.id AND tc.id = $1 ORDER BY tc.nombre ASC",
      [id]
    );
    if (TIPO_COMIDAS.rowCount > 0) {
      return res.jsonp(TIPO_COMIDAS.rows);
  ...
  public async ActualizarComida(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const datosOriginales = await pool.query("SELECT * FROM cg_tipo_comidas WHERE id = $1", [id]);
      ...
      await pool.query(
        "UPDATE cg_tipo_comidas SET nombre = $1, tipo_comida = $2, hora_inicio = $3, hora_fin = $4 " +
          "WHERE id = $5",
  ...
  public async VerUltimoRegistro(req: Request, res: Response) {
    const TIPO_COMIDAS = await pool.query(
      "SELECT MAX (id) FROM cg_tipo_comidas"
    );
    if (TIPO_COMIDAS.rowCount > 0) {
      return res.jsonp(TIPO_COMIDAS.rows);
    } else {
  ...
  public async CrearDetalleMenu(req: Request, res: Response): Promise<void> {
    try {
      ...
      await pool.query(
        "INSERT INTO detalle_menu (nombre, valor, observacion, id_menu) " +
          "VALUES ($1, $2, $3, $4)",
  ...
  public async VerUnDetalleMenu(req: Request, res: Response): Promise<any> {
    ...
    const TIPO_COMIDAS = await pool.query(
      "SELECT tc.id AS id_servicio, tc.nombre AS servicio, " +
        ...
        "WHERE tc.id = menu.tipo_comida AND dm.id_menu = menu.id AND menu.id = $1",
      [id]
    );
    if (TIPO_COMIDAS.rowCount > 0) {
      return res.jsonp(TIPO_COMIDAS.rows);
  ...
  public async ActualizarDetalleMenu(
    ...
    try {
      ...
      const datosOriginales = await pool.query(
        "SELECT * FROM detalle_menu WHERE id = $1",
        [id]
      );
      const [datos] = datosOriginales.rows;
      ...
      await pool.query(
        "UPDATE detalle_menu SET nombre = $1, valor = $2, observacion = $3 " +
          "WHERE id = $4",
