...
class AutorizacionesControlador {
    ...
    public async ActualizarEstadoAutorizacionPermiso(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const response = await pool.query('SELECT * FROM autorizaciones WHERE id_permiso = $1', [id_permiso]);
    ...
    public async ActualizarEstadoPlanificacion(req: Request, res: Response): Promise<Response> {
        ...
        if (datos === 'ok') {
            ...
            try {
                ...
                // CONSULTAR DATOS ANTES DE ACTUALIZAR PARA PODER REGISTRAR AUDITORIA
                const response = await pool.query('SELECT * FROM autorizaciones WHERE id_plan_hora_extra = $1', [id]);
                const [datos] = response.rows;
    ...
    public async ActualizarEstadoSolicitudes(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            // CONSULTAR DATOS ANTES DE ACTUALIZAR PARA PODER REGISTRAR AUDITORIA
            const response = await pool.query('SELECT * FROM autorizaciones WHERE id = $1', [id]);
            const [datos] = response.rows;
            ...
            await pool.query(`UPDATE autorizaciones SET estado = $1, id_documento = $2 WHERE id = $3`
