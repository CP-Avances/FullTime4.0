    ...
    setInterval(async () => {
        ...
        if (hora === 14) {
            ...
            const PARAMETRO = await pool.query(
                `
                SELECT * FROM ep_detalle_parametro WHERE id_parametro IN (24,25)
                `
            );
                    ...
                    const EMPLEADOS = await pool.query(
                        `
                            SELECT 
                                ...
                                mn.link, 
                                c.fecha_salida,
                                c.fecha_salida + INTERVAL '1 year' AS fecha_aniversario
                            FROM 
                            ...
                            WHERE 
                                ig.estado = 1
                                AND (c.fecha_salida + INTERVAL '1 year')::DATE = '2026-04-30';
                        `
                        , [fecha]);
