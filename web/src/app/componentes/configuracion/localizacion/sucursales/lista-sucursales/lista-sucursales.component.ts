...
export class ListaSucursalesComponent implements OnInit {
   ...
   ** ************************************************************************************************** **/
  async generarReporteSucursales(action: 'pdf'|'excel'|'csv'|'xml') {
    const data = {
    ...
    switch (action) {
      case 'pdf':
        this.reportes.generarReporteServicio('sucursales', 'pdf', data).subscribe({
      ...
      case 'excel':
        this.reportes.generarReporteServicio('sucursales', 'excel', data).subscribe({
          next: ({ blob, filename }) => FileSaver.saveAs(blob, filename),
          error: (error) => {
      ...
      case 'csv':
        this.reportes.generarReporteServicio('sucursales', 'csv', data).subscribe({
          next: ({ blob, filename }) => FileSaver.saveAs(blob, filename),
          error: (e) => {
      ...
      case 'xml':
        this.reportes.generarReporteServicio('sucursales', 'xml', data).subscribe({
          next: ({ blob, filename }) => FileSaver.saveAs(blob, filename),
          error: (e) => {
