...
export class ReporteTimbresMultiplesComponent implements OnInit, OnDestroy {
  ...
  MostrarInformacion(seleccionados: any, accion: any) {
    this.data_pdf = [];
    this.R_asistencias.ReporteTimbresMultiple(seleccionados, this.rangoFechas.fec_inico, this.rangoFechas.fec_final).subscribe(res => {
      this.data_pdf = res;
      switch (accion) {
  ...
  ExportarExcel(): void {
    ...
    xlsx.writeFile(wb, `Timbres_usuarios_${this.opcionBusqueda == 1 ? 'activos' : 'inactivos'}.xlsx`);
  }
