...
import { RegistrarEstadoCivilComponent } from '../registrar-estado-civil/registrar-estado-civil.component';
...
export class ListarEstadoCivilComponent {
  ...
  constructor(
    ...
    public restEC: EstadoCivilService,
  ...
  ListarEstadoCivil() {
    ...
    this.restEC.ListarEstadoCivil().subscribe(datos => {
      this.estadosCivil = datos;
      console.log("ver datos de estado civil", this.estadosCivil)
  ...
  AbrirVentanaEditarGenero(datosSeleccionados: any) {
    this.ventana.open(EditarEstadoCivilComponent, { width: '400px', data: datosSeleccionados })
  ...
  async generarReporteEstadosCivil(action: 'pdf'|'excel'|'csv'|'xml') {
    ...
    switch (action) {
      case 'pdf':
        this.reportes.generarReporteServicio('estado-civil', 'pdf', data).subscribe({
      ...
      case 'excel':
        this.reportes.generarReporteServicio('estado-civil', 'excel', data).subscribe({
          next: ({ blob, filename }) => FileSaver.saveAs(blob, filename),
          error: (e) => {
      ...
      case 'csv':
        this.reportes.generarReporteServicio('estado-civil', 'csv', data).subscribe({
          next: ({ blob, filename }) => FileSaver.saveAs(blob, filename),
          error: (e) => {
      ...
      case 'xml':
        this.reportes.generarReporteServicio('estado-civil', 'xml', data).subscribe({
          next: ({ blob, filename }) => FileSaver.saveAs(blob, filename),
          error: (e) => {
  ...
  async generarExcelGeneros() {
    ...
    const generos: any[] = [];
    this.estadosCivil.forEach((nivel: any, index: number) => {
      generos.push([
        index + 1,
        nivel.id,
        nivel.estado_civil,
