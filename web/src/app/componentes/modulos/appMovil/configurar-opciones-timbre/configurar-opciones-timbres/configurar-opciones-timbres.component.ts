...
export class ConfigurarOpcionesTimbresComponent implements OnInit {
  ...
  BuscarInformacionGeneral() {
    ...
    this.regimen = [];
    this.cargos = [];
    this.restUsuario.UsuariosTimbreMovilGeneral(1, true).subscribe(
      (res: any[]) => {
  ...
  RegistrarMultiple(data: any) {
    ...
    if (
      ...
      this.seleccion_internet.value === null && this.seleccion_ubicacion.value === null
    ) {
      ...
      if (data.length > 0) {
        ...
        let buscar = {
          ids_empleados: ids_empleados,
        };
        console.log('ingresa en data ', buscar);

        this.restTimbres.BuscarOpcionesMarcacion(buscar).subscribe(
          async res => {
  ...
  getGuardarConfiguracionTimbreMovil(){
    return this.tienePermiso('Guardar Configuración Timbre Móvil');
  }

  getVerConfiguracionTimbreMovil(){
    return this.tienePermiso('Ver Configuración Timbre Móvil');
  }
