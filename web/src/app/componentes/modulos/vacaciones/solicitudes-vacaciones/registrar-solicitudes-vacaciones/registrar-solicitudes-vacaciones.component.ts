...
import { SolicitudVacacionRequest } from "src/app/interfaces/SolicitudesVacacion";
import { VacacionesService } from "src/app/servicios/modulos/modulo-vacaciones/vacaciones/vacaciones.service";
...
export class RegistrarSolicitudesVacacionesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      this.form.get('fecha_inicio')?.valueChanges
        .subscribe(() => this.calcularDiasTotales())
    );
    this.subscriptions.add(
      this.form.get('fecha_final')?.valueChanges
        .subscribe(() => this.calcularDiasTotales())
    );
    this.subscriptions.add(
      this.form.get('incluir_feriados')?.valueChanges
        .subscribe(() => this.calcularDiasTotales())
    );
  }
  ...
  crearFormulario(): FormGroup {
    return this.formBuilder.group({
      id_cargo_vigente: [null, Validators.required],
      ...
      num_sabado: [0],
      num_domingo: [0],
      num_dias_totales: [0, Validators.required],
      num_horas: [0],
      user_name: [''],
      ip: [''],
      ip_local: [''],
  ...
  calcularDiasTotales(): void {
    const inicio: string = this.form.value.fecha_inicio;
    const fin: string = this.form.value.fecha_final;
    const incluirFeriados: boolean = this.form.value.incluir_feriados;
    ...
    for (let d = new Date(fechaInicio); d <= fechaFinal; d.setDate(d.getDate() + 1)) {
      const dia = d.getDay();
      if (!incluirFeriados && this.feriados.some(f => f.getTime() === d.getTime())) continue;
      diasSemana[dia] += 1;
      total += 1;
    }

    this.form.patchValue({
      num_lunes: diasSemana[1],
      num_martes: diasSemana[2],
      num_miercoles: diasSemana[3],
      num_jueves: diasSemana[4],
      num_viernes: diasSemana[5],
      num_sabado: diasSemana[6],
      num_domingo: diasSemana[0],
      num_dias_totales: total
    }, { emitEvent: false })
  }
  ...
  registrar(): void {
    ...
    const cargaUtil: SolicitudVacacionRequest = {
      id_empleado: this.idEmpleado,
      ...this.form.value
    };

    this.subscriptions.add(
      this.vacacionesService.RegistrarVacaciones(cargaUtil)
        .subscribe({
          next: () => {
            this.toastr.success('Solicitud registrada con éxito');
            this.router.navigate(['/verEmpleado', this.idEmpleado, 'solicitudes-vacaciones']);
          },
          error: (err) => {
            console.log(err);
            this.toastr.error('Error al registrar la solicitud');
          }
        })
    );
  }
