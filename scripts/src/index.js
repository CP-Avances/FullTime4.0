...
async function main() {
  ...
  if (lista_empresa.length === 0) {
    id_empresa = await empresa.setEmpresa(
      "Vaca Ortiz",
      "1783938449001",
      "Quito",
      "02293435",
      "vaca_ortiz@gmail.com",
      "Jorge Paez",
      "Pública",
      "Sucursales",
      5,
   ...
   * INSERTAR SUCURSALES
   */
  let sucursales_ids = await Promise.all(
    filtro_ciudades_Ids.map((obj) => {
      return empresa.setSucursales(
        id_empresa.establecimiento + " " + obj.ciudad,
        obj.id_ciudad,
        id_empresa.id
      );
    })
   ...
   */
  await Promise.all(
    departamentos.map(async (obj) => {
      var id = await empresa.setDepatamentoFulltime(
        obj.nombre,
        obj.nivel,
        sucursales_ids[0].id
      );
  ...
  departamentos.map((obj) => {
    let index_depa = departamentos.findIndex(
      (depa) => depa.depa_id === obj.dep_depa_id
    );
  ...
  let cg_tipo_permiso = await Promise.all(
    lista_tipo_permiso.map(async (obj) => {
      obj.id = await tipo_permi.Set_Cg_tipo_permiso(obj);
      return obj;
    })
  );
  cg_tipo_permiso.forEach((obj) => {
   ...
   */
  let array_con_Contrato = await Promise.all(
    nuevo_array.map(async (obj) => {
   ...
   */
  let array_con_Cargo = await Promise.all(
    array_con_Contrato.map(async (obj) => {
      let index_depa = departamentos.findIndex(
        (depa) => depa.depa_id === obj.depa_id
      );
      return await empleados.setCargos(
        obj,
        departamentos[index_depa].id,
        sucursales_ids[0].id
      );
    })
  );
...
async function InsertarTimbres() {
  ...
  timbres_empleado.forEach(async (ele) => {
    return await timbres.SetTimbreActual(ele);
  });
  return "Datos insertado exitosamente";
}
